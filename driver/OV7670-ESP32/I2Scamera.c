...
void gpio_matrix_in(uint8_t gpio, uint32_t signal_idx, bool inv);
...
uint16_t* camera_getLine(uint16_t lineno)
{
	...
	do{
		...
		if ((xTaskGetTickCount() - start_tick) * portTICK_PERIOD_MS > 1000) return NULL;
...
static void i2s_init()
{
	...
	gpio_matrix_in(s_config.VSYNC, I2S0I_V_SYNC_IDX,   false);
	gpio_matrix_in(0x38,           I2S0I_H_SYNC_IDX,   false);
	gpio_matrix_in(0x38,           I2S0I_H_ENABLE_IDX, false);
	gpio_matrix_in(s_config.PCLK,  I2S0I_WS_IN_IDX,    false);
