...
//	delay(1000);
	...
	// XCLOK 出力 (use new single‐call LEDC API)
	pinMode(cam_conf.XCLK, OUTPUT);
	digitalWrite(cam_conf.XCLK, LOW);

	if (!ledcAttach(cam_conf.XCLK, cam_conf.xclk_freq_hz, 2)) {
		Serial.println(F("LEDC attach failed"));
		return ESP_FAIL;  // or handle error as needed
	}

	// now drive XCLK at 50% duty
	ledcWrite(cam_conf.XCLK, (1 << (2 - 1)));  // resolution=2 → max=3, so 2/4=50%
